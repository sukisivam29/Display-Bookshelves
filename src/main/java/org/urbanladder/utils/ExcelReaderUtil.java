...
public class ExcelReaderUtil {
    ...
    private static final org.slf4j.Logger log =
            LoggerFactory.getLogger(ExcelReaderUtil.class);

    public static List<String> getExpectedMenuItems(String filePath,
                                                    String sheetName) {
    ...
    public static String getCellValue(String filePath,
                                      String sheetName,
                                      String key) {
        ...
        try (FileInputStream fis = new FileInputStream(filePath)) {
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) {
                ...
                if (keyCell != null
                        && keyCell.toString().trim().equalsIgnoreCase(key)) {
                    ...
                    if (valueCell.getCellType() == CellType.NUMERIC) {
                        return String.valueOf(
                                (long) valueCell.getNumericCellValue()
                        );
                    }
        ...
        throw new RuntimeException(
                "Key '" + key + "' not found in sheet '" + sheetName + "'"
        );
    }
}